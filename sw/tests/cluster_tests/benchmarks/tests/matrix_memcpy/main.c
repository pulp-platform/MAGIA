...
/*
 * matrix_memcpy — main core (CV32) binary. See vector_add/main.c for the
 * full rationale. A copy's own golden output IS the input (`mat[]`); PLAY's
 * data.h ships no separate `expected[]` for this kernel.
 */
...
#include "../../../../../../PLAY/test/matrix_memcpy/test_data/data.h"

#include "matrix_memcpy_pulp_task_bin.h"

#define TOLL 0.004f

#define X_BASE      (L1_BASE + 0x00000000)   /* mat copy   : DIM_M*DIM_N*4 B */
#define Z_BASE      (L1_BASE + 0x00003000)   /* result     : DIM_M*DIM_N*4 B */
...
int main(void) {
    ...
    volatile matrix_memcpy_params_t *params =
        (volatile matrix_memcpy_params_t *)PARAMS_BASE;
    params->src   = X_BASE;
    params->dst   = Z_BASE;
    params->dim_M = DIM_M;
    params->dim_N = DIM_N;
    ...
    cluster_dispatch_task_with_params(MATRIX_MEMCPY_TASK, PARAMS_BASE);
    ...
    if (errors == 0) {
        printf("[Main core %u] matrix_memcpy PASS (%d elements)\n",
               (unsigned)hartid, DIM_M * DIM_N);
    } else {
        printf("[Main core %u] matrix_memcpy FAIL (%u/%d mismatches)\n",
