...
/* vector_axpy — main core (CV32) binary. See vector_add/main.c. */

#include "magia_tile_utils.h"
#include "cluster_utils.h"

#define TARGET_IS_PULP_OPEN 1
#include "../../../../../../PLAY/test/vector_axpy/test_data/data.h"

#include "vector_axpy_pulp_task_bin.h"
...
int main(void) {
    ...
    volatile vector_axpy_params_t *params =
        (volatile vector_axpy_params_t *)PARAMS_BASE;
    params->src_a = X_BASE;
    params->src_b = Y_BASE;
    params->alpha = scalar;
    params->dst   = Z_BASE;
    ...
    cluster_dispatch_task_with_params(VECTOR_AXPY_TASK, PARAMS_BASE);
    ...
    if (errors == 0) {
        printf("[Main core %u] vector_axpy PASS (%d elements)\n",
               (unsigned)hartid, LEN);
    } else {
        printf("[Main core %u] vector_axpy FAIL (%u/%d mismatches)\n",
