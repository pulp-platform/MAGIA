 ...
 *
 * Parameters (src_a/src_b/dst/len) are passed via a runtime params struct
 * (vector_add_params_t, filled in by main.c and handed to
 * cluster_dispatch_task_with_params()) -- MAGIA's normal cluster-task
 * dispatch convention, the same one every other ported kernel under
 * sw/tests/cluster_tests/ uses. An earlier revision of this file
 * (PIC_CALL_OVERHEAD.md Sec. 11l) instead hardcoded fixed L1 addresses to
 * shave a few measurement-only instructions off `rvc`/`imiss` for the
 * PLAY comparison; reverted back to the standard struct-passing pattern
 * so this task stays consistent with the rest of the suite. See
 * PIC_CALL_OVERHEAD.md Sec. 11l/11m/11p/11q for what that experiment
 * found (still valid, just no longer reflected in this file's structure).
 */
...
static void vector_add_fork_entry(void *arg) {
    ...
    /* No-ops unless built with stats=1 (see sw/kernel_pulp/Makefile); the
     * HOTTING+REPEAT loop just calls vector_add() repeatedly, same as
     * PLAY's own run_test() would. */
    INIT_STATS();
    START_LOOP_STATS();
    START_STATS();
