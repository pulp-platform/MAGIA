...
/* vector_offset — main core (CV32) binary. See vector_add/main.c. */

#include "magia_tile_utils.h"
#include "cluster_utils.h"

#define TARGET_IS_PULP_OPEN 1
#include "../../../../../../PLAY/test/vector_offset/test_data/data.h"

#include "vector_offset_pulp_task_bin.h"
...
int main(void) {
    ...
    volatile vector_offset_params_t *params =
        (volatile vector_offset_params_t *)PARAMS_BASE;
    params->src    = X_BASE;
    ...
    cluster_dispatch_task_with_params(VECTOR_OFFSET_TASK, PARAMS_BASE);
    ...
    if (errors == 0) {
        printf("[Main core %u] vector_offset PASS (%d elements)\n",
               (unsigned)hartid, LEN);
    } else {
        printf("[Main core %u] vector_offset FAIL (%u/%d mismatches)\n",
