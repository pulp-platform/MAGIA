 ...
 * 
 * MAGIA NoC Single Source/Destination Synchronization Test
 */
...
int main(void) {
  ...
  printf("Running sigle source-sink algorithm...\n");
  ...
  for (int i = 0; i < CACHE_HEAT_CYCLES; i++) {
    if (get_hartid() % 2) { // SRC
      ...
      do {
        sync_en[get_hartid()] = mmio32(SYNC_EN + (get_hartid()-1)*L1_TILE_OFFSET);
#if VERBOSE > 100
