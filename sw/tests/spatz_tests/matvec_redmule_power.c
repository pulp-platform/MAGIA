 ...
 * 
 * Matrix-Vector Multiplication Power Test: RedMule
...
int main(void) {
    ...
    redmule_cfg((unsigned int)MAT_BASE, (unsigned int)VEC_X_BASE, (unsigned int)DST_BASE,
                16, 96, 1, (uint8_t)gemm_ops, (uint8_t)Float16);
    ...
    // =====================================================
    // VCD dump: M×N matvec (K=1 for GEMV)
    // =====================================================
    
    while (hwpe_acquire_job() < 0);
    
    redmule_cfg((unsigned int)MAT_BASE, (unsigned int)VEC_X_BASE, (unsigned int)DST_BASE,
                M_SIZE, N_SIZE, 1, (uint8_t)0x00, (uint8_t)Float16);
