 ...
 * 
 * MAGIA NoC Row Hyper Synchronization Test
 */
...
#define SYNC_MAX_BIT   (30-__builtin_clz(MESH_X_TILES))
...
int main(void) {
  ...
  printf("Running row hyper algorithm...\n");
  ...
  for (int i = 0; i < CACHE_HEAT_CYCLES; i++) {
    ...
    while ((sync_bit < SYNC_MAX_BIT) && ((tile_hartid & sync_mask) == sync_sign)){
      ...
      if (tile_xid <= SYNC_NODE_X_ID) { // Left half
...
#if VERBOSE > 10
          printf("[ASCEND PHASE] Left DST: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
...
#if VERBOSE > 10
          printf("[ASCEND PHASE] Left SRC: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
            sync_bit, sync_ohbit, sync_mask, sync_sign);
#endif
        ...
        if (!(tile_hartid & sync_ohbit)) {  // DST
#if VERBOSE > 10
          printf("[ASCEND PHASE] Right DST: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
            sync_bit, sync_ohbit, sync_mask, sync_sign);
#endif
          // Wait for SRC to request synchronization
          while (mmio32(SYNC_BASE + 4*sync_bit + tile_hartid*L1_TILE_OFFSET) < 1);
...
#if VERBOSE > 10
          printf("[ASCEND PHASE] Right SRC: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
            sync_bit, sync_ohbit, sync_mask, sync_sign);
#endif
          // Send synchronization request to DST: No need for AMOs (single source write)
          mmio32(SYNC_BASE + 4*sync_bit + (tile_hartid ^ sync_ohbit)*L1_TILE_OFFSET) = 1;
    ...
    while ((sync_bit < SYNC_MAX_BIT) && ((tile_hartid & sync_mask) == sync_sign)) {
      ...
      if (tile_xid <= SYNC_NODE_X_ID) { // Left half
...
#if VERBOSE > 10
          printf("[DESCEND PHASE] Left SRC: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
            sync_bit, sync_ohbit, sync_mask, sync_sign);
#endif
          // Send synchronization response to SRC: No need for AMOs (single source write)
          mmio32(SYNC_BASE + 4*sync_bit + (tile_hartid ^ sync_ohbit)*L1_TILE_OFFSET) = 1;
...
#if VERBOSE > 10
          printf("[DESCEND PHASE] Left DST: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
            sync_bit, sync_ohbit, sync_mask, sync_sign);
#endif
          // Wait for DST synchronization response
          while (mmio32(SYNC_BASE + 4*sync_bit + tile_hartid*L1_TILE_OFFSET) < 1);
        ...
        if (!(tile_hartid & sync_ohbit)) {  // SRC
#if VERBOSE > 10
          printf("[DESCEND PHASE] Right SRC: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
            sync_bit, sync_ohbit, sync_mask, sync_sign);
#endif
          // Send synchronization response to SRC: No need for AMOs (single source write)
          mmio32(SYNC_BASE + 4*sync_bit + (tile_hartid ^ sync_ohbit)*L1_TILE_OFFSET) = 1;
...
#if VERBOSE > 10
          printf("[DESCEND PHASE] Right DST: sync_bit - %0d, sync_ohbit - 0x%0x, sync_mask - 0x%0x, sync_sign - 0x%0x\n",
            sync_bit, sync_ohbit, sync_mask, sync_sign);
#endif
          // Wait for DST synchronization response
          while (mmio32(SYNC_BASE + 4*sync_bit + tile_hartid*L1_TILE_OFFSET) < 1);
