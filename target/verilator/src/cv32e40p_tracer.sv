// ---------------------------------------------------------------------------
// VENDORIZED COPY -- Verilator flow only.
//
// Byte-identical to cv32e40p's bhv/cv32e40p_tracer.sv at rev a8206ab except for
// the removal of its two `#` delays. The tile is built as a hierarchical
// block, and Verilator 5.046 rejects `--lib-create` combined with `--timing`
// and delays ("Unsupported: --lib-create with --timing and delays"), so the
// upstream file cannot be compiled here at all.
//
// It is picked up because the flow's makefile puts target/verilator/src on the
// include path ahead of the cv32e40p checkout's bhv/, and cv32e40p_wrapper.sv
// pulls the tracer in as `include "cv32e40p_tracer.sv"`. Both changes are
// marked "MAGIA verilator:" below. Re-vendorize on any cv32e40p bump.
// ---------------------------------------------------------------------------
...
#(
  ...
  // MAGIA verilator: was `assign #0.01 clk_i_d = clk_i;`. The delay exists to
  // order the `negedge clk_i_d` pipeline blocks after the `negedge clk_i`
  // register/memory monitor below; a plain continuous assign keeps that
  // ordering by delta. eval_comb's 0.01-wide low pulse after each posedge
  // collapses to a delta, which changes nothing: it gates an `always @*` whose
  // outputs are only ever sampled on the falling edge, where it reads 1 either
  // way.
  assign clk_i_d = clk_i;
  ...
  always begin
    ...
    // MAGIA verilator: was `#0.1ns;`. It paced the handoff to the ISS, which is
    // the commented-out `@(ovp_retire)` above; nothing waits on `retire` here.
    // The loop still blocks on `wait(trace_q.size() != 0)` once drained, so
    // dropping the delay cannot spin.
  end
