...
// =============================================================================
// GEMV Kernels (from spatzBenchmarks/gemv)
// Matrix-Vector Multiplication: c = A * b
// Based on official Spatz GEMV implementation
// Assumes column-major storage for matrix A
// =============================================================================

// 64-bit GEMV with m4 (processes 2 columns at a time)
// Computes: c[M] = A[M×N] * b[N] where A is column-major
static inline void gemv_v64b_m4(double *a, double *b, double *c, 
                                 int M, int M_core, int N) {
...
// 32-bit GEMV with m4 (processes 2 columns at a time)
// Computes: c[M] = A[M×N] * b[N] where A is column-major
static inline void gemv_v32b_m4(float *a, float *b, float *c, 
                                 int M, int M_core, int N) {
  unsigned int vl, avl = M_core;
  float *a_, *a_start = a;
  float *b_ = b;
...
// 16-bit GEMV with m4 (processes 2 columns at a time)
// Computes: c[M] = A[M×N] * b[N] where A is column-major
static inline void gemv_v16b_m4(__fp16 *a, __fp16 *b, __fp16 *c, 
                                 int M, int M_core, int N) {
  unsigned int vl, avl = M_core;
  __fp16 *a_, *a_start = a;
  __fp16 *b_ = b;
  ...
  do {
    ...
    for (int col = 0; col < N; col += 2) {
      ...
      // Multiply and accumulate (using flh for FP16 scalar load)
      float t0;
      ...
      // Multiply and accumulate (second column)
      float t1;
