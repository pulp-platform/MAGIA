 ...
 * 
 * This wrapper integrates the Spatz Core Complex (spatz_cc) from the Spatz Cluster
 * into the MAGIA tile architecture. Key features:
 * 
 */
...
#(
  ...
  // - PipeRegs[5][6]: 5 FPU blocks (FMA, DIVSQRT, NONCOMP, CONV, DOTP) × 6 formats
  // - UnitTypes: MERGED = single FPU per block, PARALLEL = separate units
  // - PipeConfig: BEFORE = registers before FPU (Spatz default)
  ...
  localparam int unsigned HCIMasterPorts      = TCDMPorts  // One HCI master per TCDM port
)(
  input  logic                                    clk_i,
  input  logic                                    rst_ni,
  ...
  // These ports connect TCDM ports from Spatz to HCI interconnect
  ...
  // Instruction Cache Interface (Hive) - Dedicated icache for spatz_cc
  ...
  // This creates: va_t, pa_t, l0_pte_t, pte_sv32_t based on AddrWidth
  ...
  typedef struct packed {
    ...
    logic acc_qvalid;
    logic acc_pready;
    logic [1:0] ptw_valid;     // 2 PTW ports (I-TLB and D-TLB)
    va_t [1:0] ptw_va;         // Virtual addresses (2 ports) - from SNITCH_VM_TYPEDEF
    pa_t [1:0] ptw_ppn;        // Physical page numbers (2 ports) - from SNITCH_VM_TYPEDEF
  } hive_req_t;
  ...
  typedef struct packed {
    ...
    acc_rsp_t acc_resp;
    logic acc_pvalid;
    logic [1:0] ptw_ready;       // 2 PTW ports
    l0_pte_t [1:0] ptw_pte;      // Page table entries (2 ports) - from SNITCH_VM_TYPEDEF
    logic [1:0] ptw_is_4mega;    // 4MB page indicator per port
  } hive_rsp_t;
  ...
  // No intermediate OBI signals needed - direct TCDM to HCI conversion
  ...
  // Uses tcdm2hci_amo which includes full atomic resolution via obi_atop_resolver
  tcdm2hci_amo #(
    .MaxTrans      ( 4                                       ),
    .AddrWidth     ( AddrWidth                               ),
    .DataWidth     ( DataWidth                               ),
