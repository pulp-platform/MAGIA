...
#(
  ...
  // Payload broadcast to every slave (whole-struct copy so channel-specific
  // optional fields are carried without being named); only the selected slave
  // sees req asserted.
  ...
  // The whole head response struct is copied first (carrying any channel
  // specific fields such as exokay), then rvalid/rdata/err/gnt are overridden.
  // ---------------------------------------------------------------------------
