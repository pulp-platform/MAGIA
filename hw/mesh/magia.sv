...
`ifdef VERILATOR
  // Packed ports are part of the fixed hierarchical tile boundary.
...
`endif
        ...
        // Forward the mesh debug request to each tile.
...
`ifdef VERILATOR
    ...
    assign vt_tile_fsync_rsp = '{default: '0};
    assign hn_tile_fsync_rsp = '{default: '0};
    assign vn_tile_fsync_rsp = '{default: '0};
`else
    // Interface ports require an instance even when the controller is absent.
    for (genvar i = 0; i < N_TILES; i++) begin: gen_tie_off
      assign ht_fsync_if[i].wake = '0;
      assign ht_fsync_if[i].lvl = '0;
      assign ht_fsync_if[i].id_rsp = '0;
      assign ht_fsync_if[i].error = '0;
      assign hn_fsync_if[i].wake = '0;
      assign hn_fsync_if[i].lvl = '0;
      assign hn_fsync_if[i].id_rsp = '0;
      assign hn_fsync_if[i].error = '0;
      assign vt_fsync_if[i].wake = '0;
      assign vt_fsync_if[i].lvl = '0;
      assign vt_fsync_if[i].id_rsp = '0;
      assign vt_fsync_if[i].error = '0;
      assign vn_fsync_if[i].wake = '0;
      assign vn_fsync_if[i].lvl = '0;
      assign vn_fsync_if[i].id_rsp = '0;
      assign vn_fsync_if[i].error = '0;
    end
`endif
  end
